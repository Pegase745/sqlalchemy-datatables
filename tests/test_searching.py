...
def test_method_none(session):
    res = get_result(
        session=session, column=User.id, search_method="none", search_value="abc"
    )
...
def test_method_numeric(session):
    res = get_result(
        session=session, column=User.id, search_method="numeric", search_value="10"
    )
...
def test_method_numeric_illegal_input(session):
    res = get_result(
        session=session, column=User.id, search_method="numeric", search_value="abc"
    )
    try:
        float("abc")
...
def test_group_by(session):
    ...
    query = session.query().select_from(User).join(Address).group_by(Address)
